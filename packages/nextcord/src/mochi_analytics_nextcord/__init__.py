...
#: nextcord and its discord.py siblings do not agree on which members exist.
...
def attach_mochi(
    ...
    *,
    ...
    """Hook a :class:`MochiClient` into a nextcord client.

    Works with ``nextcord.Client``, ``nextcord.AutoShardedClient`` and
    ...
    async def on_interaction(interaction: nextcord.Interaction) -> None:
        if not auto_track_commands:
            return
        if interaction.type is not nextcord.InteractionType.application_command:
            return
        # nextcord names this `application_command`, not `command`.
...
def wrap_command(
    ...
    def decorate(fn: Callable[..., Awaitable[Any]]) -> Callable[..., Awaitable[Any]]:
        async def wrapped(interaction: nextcord.Interaction, *args: Any, **kwargs: Any) -> Any:
...
def _add_listener(client: nextcord.Client, callback: Callable, name: str) -> None:
...
def _remove_listener(client: nextcord.Client, callback: Callable, name: str) -> None:
    remove = getattr(client, "remove_listener", None)
    if callable(remove):
        remove(callback, name)


def _full_command_name(interaction: nextcord.Interaction) -> str:
...
def _name_from_data(interaction: nextcord.Interaction) -> str:
...
def _command_source(interaction: nextcord.Interaction) -> str:
    data = getattr(interaction, "data", None) or {}
...
def _channel_type_of(interaction: nextcord.Interaction) -> str:
...
def _shard_id_of(client: nextcord.Client, guild: Optional[nextcord.Guild]) -> int:
...
def _total_shards_of(client: nextcord.Client) -> int:
...
def _snapshots(client: nextcord.Client) -> List[MochiSnapshot]:
