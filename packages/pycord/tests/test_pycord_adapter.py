...
from mochi_analytics_pycord import attach_mochi, wrap_command
...
class FakeChannel:
    def __init__(self, type_name: str) -> None:
        self.type = getattr(discord.ChannelType, type_name)
...
class FakeInteraction:
    def __init__(
        ...
        # Py-cord attaches the invoked command as `command`.
...
async def test_wrap_command_unwraps_an_application_context():
    """Py-cord passes an ApplicationContext, which wraps the interaction."""
    mochi = RecordingClient()
    ...
    await handler(FakeContext(FakeInteraction(FakeCommand("play"))))

    assert mochi.events[0].name == "play"
    assert mochi.events[0].guild_id == "42"
